...
- The city also has four fictional, mixed-member neighborhood crews with saved affiliation/standing, map territories, recruiters, home-territory sale bonuses, and rival-territory pursuit responses. It is not yet a full party, co-op, or territory-war system. Mercer Market has a static high-detail frontage and vehicle motion uses deterministic arcade steering/braking physics.
...
- `game/simulation/crews.ts`: crew definitions, territory lookup, and default reputation state.
- `game/render/market-art.ts`, `vehicle-physics.ts`: static storefront detail and deterministic vehicle motion. `game/ui/MiniMap.ts`: HUD-map renderer.
